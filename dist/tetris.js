...
*/
        var tetrisCanvas = {
          ...
          drawPreviewShape: function (shape) {
            ...
            var startX =
              (this.preview.width - gsize * shape.getColumnCount()) / 2;
            var startY =
              (this.preview.height - gsize * shape.getRowCount()) / 2;
            for (var i = 0; i < matrix.length; i++) {
              for (var j = 0; j < matrix[i].length; j++) {
                var value = matrix[i][j];
                if (value === 1) {
...
// Membuat objek audio untuk backsound
const backgroundSound = new Audio("audio/jellyfish jam.mp3");

// Mengatur audio untuk loop dan volume
backgroundSound.loop = true;
backgroundSound.volume = 0.5; // Sesuaikan volume sesuai kebutuhan

// Memainkan backsound ketika halaman dimuat
window.onload = function () {
  backgroundSound.play();
};

// Mencoba untuk memutar audio saat halaman dimuat
window.onload = function () {
  backgroundSound.play().catch((error) => {
    // Jika autoplay diblokir, menunggu interaksi pertama pengguna
